...
async function loadProofRecords(userId: string, db: DrizzleDB) {
  const [profile, documentRecords, sponsors, activeCertificate] = await Promise.all([
    ...
    db.query.sponsorships.findMany({
      ...
      columns: { amountKobo: true, currency: true, updatedAt: true, sponsorId: true },
    }),
...
function summarizeDocuments(documentRecords: { status: string; updatedAt: Date }[]) {
  const approvedDocumentCount = documentRecords.filter((r) => r.status === 'approved').length;
  const pendingDocumentCount = documentRecords.filter(
    (r) => r.status === 'pending' || r.status === 'more_info_requested',
  ).length;
...
function summarizeSponsors(sponsors: { sponsorId: string; amountKobo: number; currency: string }[]) {
  const uniqueSponsorCount = new Set(sponsors.map((s) => s.sponsorId)).size;
  const committedAmountKobo = sponsors.reduce((total, s) => total + s.amountKobo, 0);
...
function collectActivityTimestamps({
  ...
  return [
    profileUpdatedAt,
    certificateIssuedAt,
    ...documentRecords.map((r) => r.updatedAt),
    ...sponsors.map((s) => s.updatedAt),
