...
export default async function CommissionsPage({ params }: PageProps) {
  ...
  return (
    <section className="space-y-8">
      <PageHeader title="Commissions" subtitle="Earnings and payouts" />
      <div className="flex flex-col items-center gap-4 rounded-xl border border-border bg-card py-20 text-center">
        <Coins className="size-10 text-muted-foreground/40" aria-hidden="true" />
        <p className="text-sm font-medium text-foreground">This page is being built</p>
        <p className="max-w-xs text-xs text-muted-foreground">
          Full functionality for <strong>Commissions</strong> is coming soon. Your navigation and layout are fully wired.
        </p>
      </div>
    </section>
  );
