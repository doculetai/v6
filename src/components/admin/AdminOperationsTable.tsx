...
export function AdminOperationsTable({
  ...
  const lockCopy = adminCopy.itemLock;
          ...
          <tbody>
            {rows.map((row) => {
              ...
              const isLockedByOther =
                !!row.lockedBy && currentAdminId != null && row.lockedBy !== currentAdminId;
              return (
                <tr
                  key={row.id}
                  className={cn(
                    ...
                    <div className="flex flex-col items-start gap-1">
                      {isLockedByOther && (
                        <span className="text-xs text-muted-foreground">
                          {lockCopy.beingReviewed}
                          {row.lockedMinutesAgo != null && (
                            <> &middot; {lockCopy.minutesAgo(row.lockedMinutesAgo)}</>
                          )}
                        </span>
                      )}
