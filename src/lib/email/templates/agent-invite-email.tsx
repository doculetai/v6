...
type AgentInviteEmailProps = {
  signupUrl: string;
  agentName: string | null;
};

export function AgentInviteEmail({ signupUrl, agentName }: AgentInviteEmailProps) {
  return (
    <div style={wrapperStyle}>
      <div style={cardStyle}>
        <h1 style={headingStyle}>{emailCopy.agentStudentInvite.heading}</h1>
        <p style={bodyStyle}>{emailCopy.agentStudentInvite.body}</p>
        {agentName ? (
          <p style={agentInfoStyle}>
            Invited by: <strong>{agentName}</strong>
          </p>
        ) : null}
        <a href={signupUrl} style={ctaStyle}>
          {emailCopy.agentStudentInvite.ctaLabel}
        </a>
        <p style={footerStyle}>{emailCopy.agentStudentInvite.footer}</p>
      </div>
    </div>
  );
}
