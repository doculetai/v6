...
name: distill
description: >
  Strip a feature to its essential information by removing unnecessary complexity,
  redundant UI, and over-built scope. Use when a page feels overcrowded, has too
  many cards/sections competing for attention, shows data the user doesn't need,
  or was flagged by /product-owner for Scope Drift (over-built). In Doculet,
  precision over decoration — remove anything that doesn't serve the user's
  immediate goal. Trigger phrases: "too much on the page", "overcrowded",
  "information overload", "remove the noise", "feels cluttered", "simplify this",
  "scope drift", "over-built".
user-invokable: true
args:
  - name: target
    description: The feature or page to distill (optional)
    required: false
---
