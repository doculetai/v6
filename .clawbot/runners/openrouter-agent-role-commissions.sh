...
PROMPT_FILE="/Users/gm/v6/.clawbot/prompts/openrouter-agent-role-commissions.txt"
case "openrouter" in
  codex)
    # --full-auto: workspace-write sandbox, no per-command approval prompts
    exec codex --full-auto --model "qwen/qwen3-coder-next" -c "model_reasoning_effort=high" "$(cat "$PROMPT_FILE")"
    ;;
  claude)
    # --dangerously-skip-permissions: no tool-approval prompts — fully autonomous
    unset CLAUDECODE
    exec claude --model "qwen/qwen3-coder-next" --dangerously-skip-permissions -p "$(cat "$PROMPT_FILE")"
    ;;
  openclaw)
  ...
  openrouter)
    ...
    exec python3 - "$PROMPT_FILE" "qwen/qwen3-coder-next" <<'PY'
import sys, os
...
PY
  ...
  gemini)
    ...
    exec gemini --model "qwen/qwen3-coder-next" "$(cat "$PROMPT_FILE")"
    ;;
  ollama)
    # Local Ollama — no API key needed, no cost.
    exec ollama run "qwen/qwen3-coder-next" "$(cat "$PROMPT_FILE")"
    ;;
  openhands)
    ...
    exec python3 -m openhands.core.main       --task "$(cat "$PROMPT_FILE")"       --workspace-base "$(pwd)"       --runtime local       --model "qwen/qwen3-coder-next"       --max-iterations 50
    ;;
  *)
