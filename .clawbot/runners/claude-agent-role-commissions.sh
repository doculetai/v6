#!/bin/zsh
set -euo pipefail
cd "/Users/gm/v6/.worktrees/agent-role-commissions"
PROMPT_FILE="/Users/gm/v6/.clawbot/prompts/claude-agent-role-commissions.txt"
case "claude" in
  codex)
    # --full-auto: workspace-write sandbox, no per-command approval prompts
    exec codex --full-auto --model "claude-sonnet-4-6" -c "model_reasoning_effort=high" "$(cat "$PROMPT_FILE")"
    ;;
  claude)
    # --dangerously-skip-permissions: no tool-approval prompts — fully autonomous
    unset CLAUDECODE
    exec claude --model "claude-sonnet-4-6" --dangerously-skip-permissions -p "$(cat "$PROMPT_FILE")"
    ;;
  openclaw)
  ...
  openrouter)
    ...
    exec python3 - "$PROMPT_FILE" "claude-sonnet-4-6" <<'PY'
import sys, os
...
PY
  ...
  gemini)
    ...
    exec gemini --model "claude-sonnet-4-6" "$(cat "$PROMPT_FILE")"
    ;;
  ollama)
    # Local Ollama — no API key needed, no cost.
    exec ollama run "claude-sonnet-4-6" "$(cat "$PROMPT_FILE")"
    ;;
  openhands)
    ...
    exec python3 -m openhands.core.main       --task "$(cat "$PROMPT_FILE")"       --workspace-base "$(pwd)"       --runtime local       --model "claude-sonnet-4-6"       --max-iterations 50
    ;;
  *)
    echo "Unsupported agent: claude" >&2
    exit 1
    ;;
esac
